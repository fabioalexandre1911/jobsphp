<?php  
	...
	$filter['id_agend'] = $_POST['filter'];

	# Excluindo os dados no banco
	$manager = new Manager();

	$manager->delete_common("tb_agends",$filter,NULL);
	...
	fwrite($archive, $user->name." deletou um agendmento");
	fwrite($archive, "\n");
	...
	# Redirecionamento
	header("location: $project_index".$user->profile_page.".php?option=list_diaries&success=delete_ok");
?>